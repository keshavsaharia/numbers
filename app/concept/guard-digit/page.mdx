...
One or more guard digits can be used in floating-point arithmetic to extend the **significand** (also called mantissa) during internal computations. [IEEE 754](/spec/ieee/754) arithmetic uses guard digits to improve accuracy during subtraction of nearly equal numbers, ensure correct rounding, and avoid [catastrophic cancellation](/concept/cancellation).
