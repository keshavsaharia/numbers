...
which implies that *none* of the digits in the result are reliable. Or put differently, all $$p$$ digits can be corrupted when subtracting two nearly equal numbers.

## Guard Digits

To reduce this problem, floating-point implementations often use a [guard digit](/concept/guard-digit) — an extra digit retained during intermediate steps of subtraction. This extra digit helps preserve accuracy when the result is small due to cancellation.
