...
There are simpler ways to describe error like [relative error](/concept/relative-error) and [absolute error](/concept/absolute-error), but in the case of floating point arithmetic, it's often useful to account for the density of representable numbers around an expected result because it grows exponentially sparse for larger numbers.

<FloatDistribution fp={ fp8.e4m3 } bins={ 100 }/>

## Zeroes, `NaN` and `Infinity`

The `E4M3` format also allows two possible representations of zero (`00000000` and `10000000`) and two possible representations of `NaN` (not a number). Some floating point formats also define special values to represent positive and negative `Infinity`. One criteria for considering a floating point format to be [IEEE 754 compliant](/spec/ieee/754) is whether it can represent `NaN`, `+Infinity`, and `-Infinity`. 
...
Suppose we want to calculate $$\ln(x)$$ - we could use the [Taylor expansion](https://en.wikipedia.org/wiki/Taylor_series) of $$ln(x)$$ around $$x = 1$$.

$$
\ln(x) = \sum_{n=1}^{\infty} (-1)^{n+1} \frac{(x-1)^n}{n}
\quad \text{for} \quad 0 < x \leq 2
$$

```javascript
function ln(x, terms = 100) {
    // Boundary conditions
    if (x < 0)
        throw new Error('complex numbers not supported')
    if (x > 2) 
        throw new Error('only works for 0 < x <= 2 for now')
...
We can then employ this logarithm identity to generalize this solution for all possible values of $$x$$.

$$ \ln(ab) = \ln(a) + \ln(b) $$
$$ \ln(x) = \ln\left(\frac{x}{a}\right) + \ln(a) $$

If we choose a value of $$a$$ which is the $$k$$-th power of 2 (i.e. $$a = 2^k$$), then we can simplify this further into an algorithm that only needs to store the precomputed value of $$ln(2)$$.

$$ \ln(x) = \ln\left(\frac{x}{2^k}\right) + \ln(2^k) = \ln\left(\frac{x}{2^k}\right) + k \cdot \ln(2) $$
...
A floating point number system $$F$$ is a subset of the real numbers $$\mathbb{R}$$ (i.e. $$F \subset \mathbb{R}$$) where elements have the form $$ y = \pm m \times \beta^{e-t} $$. There are four integer parameters here which characterize any floating point number system:
- the base $$\beta$$ (sometimes called the radix), which in this case is $$2$$
- the exponent $$e$$ which lies in an **exponent range** $$e_{min} \leq e \leq e_{max}$$
- the precision $$t$$, which is subtracted from the exponent $$e$$
- the significand $$m$$, where $$0 \leq m \leq \beta^t - 1$$

 To ensure a unique representation for each nonzero $$y \in F$$, it is assumed that $$m \geq \beta^{t-1}$$ if $$y \neq 0$$, so that the system is **normalized**. The number $$0$$ is a special case in that it does not have a normalized representation.

The range of the nonzero floating point numbers in $$F$$ is given by $$ \beta^{e_{min}-1} \leq |y| \leq \beta^{e_{max}}(1 - \beta^{-t}) $$. 

Note that an alternative (and more common) way of expressing $$y$$ is:

$$ y = \pm \beta^e \times (d_1 + \frac{d_2}{\beta} + ... + \frac{d_t}{\beta^t}) = \pm \beta^e \times .d_1d_2...d_t $$

where each digit $$d_i$$ satisfies $$0 \leq d_i \leq \beta - 1$$, and $$d_1 \neq 0$$ for normalized numbers. While this "positional" representation is more common, we prefer the more concise first representation, which we usually find easier to work with. The first representation has pedagogical advantages, being entirely integer-based and therefore simpler to grasp. In the positional representation, $$d_1$$ is called the most significant digit and $$d_t$$ the least significant digit.
