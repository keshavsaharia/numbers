import { encodings } from "@/data/string/encodings";
...
import { getGroupMetadata } from '@/data/metadata'

export const metadata = getGroupMetadata(stringNavigation)

<TopNavigation id="string" group={ stringNavigation } />
...
The formal mathematical definition of a string is *a sequence of characters from a finite alphabet*. The set of all strings over an alphabet $$\Sigma$$ is denoted as $$\Sigma^*$$.

<Center>
$$ \Sigma^* = \{ \epsilon, a, aa, ab, ba, abb, \ldots \} $$
</Center>

The empty string $$\epsilon$$ is a string of length 0. Just as it is useful to have the number 0 when mathematically reasoning about numbers, it is useful to have the empty string when reasoning about strings.

<SectionListBox { ...encodings } />
